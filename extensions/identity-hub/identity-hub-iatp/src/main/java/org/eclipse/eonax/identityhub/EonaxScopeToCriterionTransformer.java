...
public class EonaxScopeToCriterionTransformer implements ScopeToCriterionTransformer {
    ...
    protected Result<String[]> tokenize(String scope) {
        ...
        if (!EONAX_VC_TYPE_SCOPE_ALIAS.equalsIgnoreCase(tokens[0])) {
            return failure("Scope alias MUST be %s but was %s".formatted(EONAX_VC_TYPE_SCOPE_ALIAS, tokens[0]));
        }
